...
module.exports = function () {
  ...
  const setPackageJson = ({ projectName, version }) => {
    const packageJson = require(path.resolve(cwd, `${projectName}/package.json`));
    packageJson.name = projectName
    packageJson.version = version
    fs.writeFileSync(path.resolve(cwd, `${projectName}/package.json`), JSON.stringify(packageJson, null, 2), {}) // \t为制表符, 2代表空格数
  }
  ...
  const inquirerHandler = async answer => {
    ...
    const realProjectName = inCurrentDir ? path.relative('../', cwd) : _projectName;
